...
function angle(a:{x:number;y:number},b:{x:number;y:number},c:{x:number;y:number}){
 ...
 return Math.acos(clamp((u.x*v.x+u.y*v.y)/(Math.hypot(u.x,u.y)*Math.hypot(v.x,v.y)),-1,1))*180/Math.PI;
}
...
export default function MirrorPage(){
 const video=useRef<HTMLVideoElement>(null),canvas=useRef<HTMLCanvasElement>(null),tracker=useRef<PoseLandmarker|null>(null),raf=useRef(0),lastSent=useRef(0),smooth=useRef<Joints|null>(null);
 ...
 useEffect(()=>{const chat=localStorage.getItem("meccanoid.endpoint")||"";setEndpoint(localStorage.getItem("meccanoid.mirrorEndpoint")||chat.replace(/\/chat\/?$/,"/mirror_pose"));setToken(localStorage.getItem("meccanoid.token")||"");return()=>stop()},[]);
 function stop(){cancelAnimationFrame(raf.current);video.current?.srcObject instanceof MediaStream&&video.current.srcObject.getTracks().forEach(t=>t.stop());setRunning(false);setStatus("Camera stopped")}
 async function start(){
  ...
  }catch(e){setStatus(e instanceof Error?e.message:"Unable to start camera");stop()}
 }
 function loop(){
  ...
  const result=t.detectForVideo(v,performance.now());if(result.landmarks[0]){const l=result.landmarks[0];new DrawingUtils(ctx).drawConnectors(l,PoseLandmarker.POSE_CONNECTIONS,{color:"#4fd8e0",lineWidth:3});
   ...
   if(endpoint&&performance.now()-lastSent.current>100){lastSent.current=performance.now();fetch(endpoint,{method:"POST",headers:{"Content-Type":"application/json",...(token?{Authorization:`Bearer ${token}`}:{})},body:JSON.stringify({joints})}).then(r=>{if(!r.ok)throw new Error(`Mirror endpoint HTTP ${r.status}`)}).catch(e=>setStatus(e.message))}
